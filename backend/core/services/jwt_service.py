...
class JWTService: # for every user will be created token and into token payload will be saved user id
    ...
    @staticmethod
    def verify_token(token, token_class: ActionTokenClassType):# validation of token with same token class
        try:
        ...
        return get_object_or_404(UserModel, pk=user_id) # if token validated we return user